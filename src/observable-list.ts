...
export class ObservableList<Value> implements Collection<Value> {
	...
	static fromObject<Value>(values: IndexedObject<Value>) {
		const list = new ObservableList<Value>();
	...
	addedEvent = new Event<ListManipulationInfo<Value>>();
	removedEvent = new Event<ListManipulationInfo<Value>>();
